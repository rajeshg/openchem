...
export class AlkylStrategy extends BaseSubstituentStrategy {
  ...
  matches(ctx: TokenContext): boolean {
    ...
    // Reject if there are aromatic/heterocyclic substituents (too complex for simple alkyl strategy)
    const aromaticPatterns = [
      "anilino",
      "phenyl",
      "benzoyl",
      "benzyl",
    ...
    return (
      ...
      !hasComplexSuffix &&
      !hasOxySuffix &&
      !hasAromaticSubstituent &&
      !hasCyclicEtherParent
    );
  }
  ...
  private applySubstituent(builder: MoleculeGraphBuilder, atomIdx: number, value: string): void {
    if (value === "hydroxy" || value === "hydroxyl") {
      ...
      builder.addTripleBond(cIdx, nIdx);
    }
  }
}
