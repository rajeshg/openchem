...
import type { ParentStructure, FunctionalGroup, StructuralSubstituent } from "../../types";
...
export function buildEsterWithRingAlkylGroup(
  ...
  const parentSubstituents = (parentStructure as { substituents?: unknown[] }).substituents || [];
  ...
  const filteredParentSubstituents = parentSubstituents.filter((sub: unknown) => {
  ...
  if (process.env.VERBOSE) {
    ...
    if (process.env.VERBOSE) {
      console.log("[buildEsterWithRingAlkylGroup] parentStructure.ring:", parentStructure.ring);
    }
  }

  if (esterOxygen && parentStructure.ring) {
    const esterOxygenId = typeof esterOxygen === "number" ? esterOxygen : esterOxygen.id;
    const ringAtomIds = new Set(parentStructure.ring.atoms.map((a: Atom) => a.id));
    ...
    if (process.env.VERBOSE) {
      console.log("[buildEsterWithRingAlkylGroup] esterOxygenId:", esterOxygenId);
      if (process.env.VERBOSE) {
        console.log("[buildEsterWithRingAlkylGroup] ringAtomIds:", Array.from(ringAtomIds));
      }
      if (process.env.VERBOSE) {
        console.log(
  ...
  if (process.env.VERBOSE) {
    console.log("[buildEsterWithRingAlkylGroup] ringAttachmentPosition:", ringAttachmentPosition);
  }
  ...
  const processedParentSubstituents = filteredParentSubstituents.map((sub: unknown) => {
    const s = sub as {
      type: string;
      name?: string;
      locant?: number;
  ...
  for (const [_, group] of groupedSubstituents) {
    ...
    const multiplier =
      group.locants.length > 1
        ? getSimpleMultiplier(group.locants.length, opsinService ?? getSharedOPSINService())
        : "";
  ...
  const isPolycyclic =
    ringName.includes("bicyclo") || ringName.includes("tricyclo") || ringName.includes("spiro");
 ...
 */
export function getAcylChainLength(esterGroup: FunctionalGroup, molecule: Molecule): number {
  if (process.env.VERBOSE) {
    console.log("[getAcylChainLength] esterGroup.atoms:", esterGroup.atoms);
    if (process.env.VERBOSE) {
      console.log("[getAcylChainLength] esterGroup.atoms.length:", esterGroup.atoms?.length);
    }
  }
...
function getAcylSubstituents(
  ...
  const carbonylBond = molecule.bonds.find(
    (b: Bond) =>
      b.type === "double" &&
      ((b.atom1 === carbonylCarbonId && molecule.atoms[b.atom2]?.symbol === "O") ||
        (b.atom2 === carbonylCarbonId && molecule.atoms[b.atom1]?.symbol === "O")),
  );
  ...
  if (process.env.VERBOSE) {
    console.log(`[getAcylSubstituents] Found ${substituents.length} substituents:`, substituents);
  }
    ...
    if (currentAtom?.symbol === "C") {
      ...
      for (const bond of molecule.bonds) {
        if (bond.atom1 === currentId || bond.atom2 === currentId) {
          ...
          if (otherAtom?.symbol === "C" && !visited.has(otherId) && otherId !== sulfurId) {
            queue.push(otherId);
          }
        }
      }
 ...
 */
function findLongestPathFrom(startId: number, allowedIds: number[], molecule: Molecule): number[] {
  const visited = new Set<number>();
  const path: number[] = [];
...
function generateAlkylNameFromPath(path: number[], molecule: Molecule): string {
  ...
  const baseName = path.length < prefixes.length ? prefixes[path.length] : `C${path.length}`;
...
function nameAcylAlkylSubstituent(
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    const bonds = molecule.bonds.filter((b) => b.atom1 === chainAtomId || b.atom2 === chainAtomId);
    ...
    for (const bond of bonds) {
      ...
      if (otherAtom.symbol === "C") {
        ...
        const subChain = findLongestChainFrom(otherAtomId, chainAtomId, molecule);
        ...
        if (process.env.VERBOSE) {
          console.log(`[nameAcylAlkylSubstituent] Found ${subName} at position ${locant}`);
        }
      }
    }
  }
  ...
  const sortedNames = Array.from(substByName.keys()).sort((a, b) => a.localeCompare(b));
...
function getAlkylSubstituentName(carbonCount: number): string {
  const names = ["", "methyl", "ethyl", "propyl", "butyl", "pentyl", "hexyl", "heptyl", "octyl"];
  return carbonCount < names.length && names[carbonCount] ? names[carbonCount]! : `C${carbonCount}`;
}
...
function detectBenzeneRingSubstituents(
  ...
  for (let i = 0; i < ring.atoms.length; i++) {
    ...
    const externalBonds = molecule.bonds.filter(
      (b) => (b.atom1 === ringAtomId || b.atom2 === ringAtomId) && b.type === "single",
    );

    for (const bond of externalBonds) {
      const externalAtomId = bond.atom1 === ringAtomId ? bond.atom2 : bond.atom1;
      ...
      if (externalAtom.symbol === "N") {
        ...
        const carbonylBond = molecule.bonds.find(
          (b) => (b.atom1 === externalAtomId || b.atom2 === externalAtomId) && b.type === "single",
        );

        if (!carbonylBond) continue;

        const carbonylCarbonId =
          carbonylBond.atom1 === externalAtomId ? carbonylBond.atom2 : carbonylBond.atom1;
        ...
        const doubleBond = molecule.bonds.find(
          (b) =>
            (b.atom1 === carbonylCarbonId || b.atom2 === carbonylCarbonId) && b.type === "double",
        );
        ...
        const acylName = nameAcylChainForAmide(carbonylCarbonId, externalAtomId, molecule);
...
function nameAcylChainForAmide(
  ...
  const chainBonds = molecule.bonds.filter(
    (b) => (b.atom1 === carbonylCarbonId || b.atom2 === carbonylCarbonId) && b.type === "single",
  );
  ...
  function findLongestChain(startId: number, visited: Set<number>): number[] {
    ...
    const neighbors = molecule.bonds
      .filter((b) => (b.atom1 === startId || b.atom2 === startId) && b.type === "single")
  ...
  const firstCarbonNeighbors = molecule.bonds
    .filter((b) => (b.atom1 === chainStartId || b.atom2 === chainStartId) && b.type === "single")
  ...
  const methylBranches = branches.filter((branchId) => {
    const branchBonds = molecule.bonds.filter((b) => b.atom1 === branchId || b.atom2 === branchId);
  ...
  if (methylBranches.length > 0) {
    const multiplier =
      methylBranches.length === 2 ? "di" : methylBranches.length === 3 ? "tri" : "";
...
export function buildEsterWithRingAcylGroup(
  ...
  if (isAromatic && ringName.includes("benzen")) {
    ...
    const ringSubstituents = ((parentStructure.substituents as StructuralSubstituent[]) || [])
      .filter((sub) => {
    ...
    if (process.env.VERBOSE) {
      console.log("[buildEsterWithRingAcylGroup] About to call detectBenzeneRingSubstituents");
    }
    let additionalSubs: Array<{ position: string; name: string }> = [];
    try {
      additionalSubs = detectBenzeneRingSubstituents(parentStructure, esterGroup, molecule);
      if (process.env.VERBOSE) {
        console.log(
    ...
    if (process.env.VERBOSE) {
      console.log("[buildEsterWithRingAcylGroup] ringSubstituents (initial):", ringSubstituents);
      if (process.env.VERBOSE) {
        console.log("[buildEsterWithRingAcylGroup] additionalSubs:", additionalSubs);
      }
      if (process.env.VERBOSE) {
        console.log("[buildEsterWithRingAcylGroup] allSubstituents:", allSubstituents);
      }
    }
    ...
    if (allSubstituents.length > 0) {
      const parts = allSubstituents.map((sub) => `${sub.position}-(${sub.name})`);
      substituentPrefix = parts.join("-");
    }

    // Build acyl name: benzoate with substituents
    const baseAcylName = "benzoate";
    const acylName = substituentPrefix ? `${substituentPrefix}${baseAcylName}` : baseAcylName;

    // Now find the alkoxy group
    const alkoxyName = getAlkoxyGroupName(esterGroup, molecule, functionalGroups, opsinService);
  ...
  function dfs(currentId: number, visited: Set<number>, path: number[]): void {
    ...
    for (const bond of molecule.bonds) {
      if (bond.atom1 === currentId || bond.atom2 === currentId) {
        ...
        if (otherId === excludeNeighborId || visited.has(otherId) || otherAtom?.symbol !== "C") {
          continue;
        }
...
export function buildEsterName(
  ...
  if (parentStructure.type === "ring" && !parentStructure.chain) {
    ...
    let isRingOnAcylSide =
      carbonylCarbonIdTemp !== undefined && ringAtomIds.has(carbonylCarbonIdTemp);
    ...
    if (isRingOnAcylSide) {
      ...
      if (process.env.VERBOSE)
        console.log("[buildEsterName] Ring is on acyl side, calling buildEsterWithRingAcylGroup");
      ...
      if (process.env.VERBOSE)
        console.log("[buildEsterName] Ring is on alkoxy side, building ring-based alkyl group");
  ...
  if (!parentStructure.chain) {
    if (process.env.VERBOSE)
      console.log("[buildEsterName] ERROR: No chain in parentStructure, returning fallback");
  ...
  if (isMultiester) {
    return buildDiesterName(parentStructure, esterGroup, molecule, functionalGroups);
  }
  ...
  if (!carbonylCarbonId || !esterOxygenId) {
    if (process.env.VERBOSE)
      console.log("[buildEsterName] Fallback: searching for carbonyl and ester oxygen");
    for (const bond of molecule.bonds) {
      if (bond.type === "double") {
  ...
  if (!alkoxyCarbonId) {
    if (process.env.VERBOSE)
      console.log("[buildEsterName] ERROR: Could not find alkoxyCarbonId, returning fallback");
    return "alkyl carboxylate";
  }
  ...
  const parentChainIncludesAlkoxy = parentChainCarbonIds.includes(alkoxyCarbonId);
  const parentChainIncludesAcyl = parentChainCarbonIds.includes(carbonylCarbonId);

  if (process.env.VERBOSE) {
    console.log("[buildEsterName] parentChainCarbonIds:", parentChainCarbonIds);
    if (process.env.VERBOSE) {
      console.log("[buildEsterName] parentChainIncludesAlkoxy:", parentChainIncludesAlkoxy);
    }
    if (process.env.VERBOSE) {
      console.log("[buildEsterName] parentChainIncludesAcyl:", parentChainIncludesAcyl);
    }
  }
  ...
  if (parentChainIncludesAcyl && parentStructure.chain) {
    ...
    if (process.env.VERBOSE) {
      console.log("[buildEsterName] Using parent chain for acyl side, length:", acylLength);
    }
  } else {
    ...
    const longestChain = findLongestChainFrom(carbonylCarbonId, esterOxygenId, molecule);
    acylLength = longestChain.length;

    if (process.env.VERBOSE) {
      console.log("[buildEsterName] Using longest chain for acyl side, length:", acylLength);
      if (process.env.VERBOSE) {
  ...
  if (parentChainIncludesAlkoxy && parentStructure.chain) {
    ...
    if (process.env.VERBOSE) {
      console.log("[buildEsterName] Using parent chain for alkoxy side, length:", alkoxyLength);
    }
  } else {
    // Use longest chain finder for alkoxy side
    // Find the longest carbon chain starting from alkoxy carbon,
    // excluding the ester oxygen direction
    const longestChain = findLongestChainFrom(alkoxyCarbonId, esterOxygenId, molecule);
    alkoxyLength = longestChain.length;
    alkoxyCarbonIds = new Set(longestChain);

    if (process.env.VERBOSE) {
      console.log("[buildEsterName] Using longest chain for alkoxy side, length:", alkoxyLength);
      if (process.env.VERBOSE) {
            ...
            for (const cid of Array.from(alkoxyCarbonIds)) {
              for (const b of molecule.bonds) {
                ...
                const neigh = molecule.bonds
                  .filter(
                    (bb: Bond) =>
                      (bb.atom1 === other || bb.atom2 === other) && bb.type === "single",
                  )
                  .map((bb: Bond) => (bb.atom1 === other ? bb.atom2 : bb.atom1))
                  .map((id: number) => molecule.atoms[id]);
                if (neigh.some((na: Atom | undefined) => na && na.symbol === "Si")) {
  ...
  if (parentChainIncludesAlkoxy && parentStructure.chain) {
    ...
    const baseAlkylName =
      chainLength < alkylChainNames.length ? alkylChainNames[chainLength]! : `C${chainLength}-yl`;
    ...
    const needsReversal = alkoxyIndexInChain === parentChainCarbonIds.length - 1;
    ...
    const substByPosition = new Map<number, { name: string; count: number }[]>();
    ...
    for (const s of alkylSubs) {
      ...
      const newLocant = needsReversal ? chainLength - originalLocant + 1 : originalLocant;
      ...
      if (name) {
        ...
        const existing = substByPosition.get(newLocant)!.find((x) => x.name === name);
        if (existing) {
          existing.count++;
        } else {
    ...
    if (process.env.VERBOSE) {
      console.log("[buildEsterName] Adding FG substituents, chainAtomIds:", chainAtomIds);
      if (process.env.VERBOSE) {
        console.log(
    ...
    for (const fg of functionalGroups) {
      ...
      if (process.env.VERBOSE) {
        console.log(`[buildEsterName] FG type=${fg.type}, prefixName=${prefixName}`);
      }
      ...
      for (let i = 0; i < chainAtomIds.length; i++) {
        ...
        for (const bond of molecule.bonds) {
          ...
          let isMatch = false;
          if (fg.type === "alcohol" && heteroAtom.symbol === "O" && heteroAtom.hydrogens === 1) {
            isMatch = true;
          ...
          if (isMatch && !chainAtomIds.includes(heteroAtomId)) {
            ...
            const existing = substByPosition.get(position)!.find((x) => x.name === prefixName);
            if (!existing) {
              substByPosition.get(position)!.push({ name: prefixName, count: 1 });
              if (process.env.VERBOSE) {
                console.log(
    ...
    const substByName = new Map<string, Array<{ position: number; count: number }>>();
    ...
    const alkylSubstituentsPrefix = alkylSubParts.length > 0 ? alkylSubParts.join("-") : "";
    ...
    if (process.env.VERBOSE) {
      console.log("[buildEsterName] Built alkyl name from parent chain:", alkylName);
    }
  } else {
    ...
    alkylName = getAlkoxyGroupName(esterGroup, molecule, functionalGroups, opsinService);
  }

  // Prefer deriving the acyl name from the parentStructure.chain when the parent chain contains the carbonyl carbon
  let acylName = "";
  try {
    const parentChainAtoms = (parentStructure.chain?.atoms || []).map((a: Atom) =>
      a && typeof a === "object" && typeof a.id === "number" ? a.id : a,
    );
    const parentIsAcyl =
      carbonylCarbonId !== undefined && parentChainAtoms.includes(carbonylCarbonId);

    if (parentIsAcyl) {
      if (process.env.VERBOSE)
        console.log("[buildEsterName] parentIsAcyl true, parentChainAtoms:", parentChainAtoms);
      // Count carbon atoms in parent chain
      const carbonCount = (parentStructure.chain?.atoms || []).filter((a: Atom) => {
        const atomId =
          a && typeof a === "object" && typeof a.id === "number"
            ? a.id
            : typeof a === "number"
      ...
      if (process.env.VERBOSE)
        console.log("[buildEsterName] computed carbonCount from parent chain:", carbonCount);
      ...
      const base =
        carbonCount < baseNames.length ? baseNames[carbonCount] : `C${carbonCount}-alkane`;
      // Special case: 1-carbon esters use "formate" not "methanoate"
      const baseAnoate = carbonCount === 1 ? "formate" : (base || "").replace("ane", "anoate");
      ...
      if (process.env.VERBOSE) {
        console.log("[buildEsterName] acylSubstituentName:", acylSubstituentName);
      }
      ...
      acylName = acylSubstituentName ? `${acylSubstituentName}${baseAcylName}` : baseAcylName;
    }
  } catch (_e) {
    const acylNames = [
      "",
...
export function buildDiesterName(
  ...
  if (allSameLength && alkoxyLengths.length > 0) {
    ...
    const baseName =
      len < alkylNames.length && len >= 0 ? (alkylNames[len] ?? "methyl") : `C${len}-alkyl`;
  ...
  const substituents = (chain.substituents || []).filter((sub: unknown) => {
    ...
    const isAlkoxySubstituent = ((sub as Record<string, unknown>).type as string)?.includes("oxy"); // methoxy, ethoxy, propoxy, etc.
  ...
  const chainAtomIds = new Set<number>(chain.atoms?.map((a: Atom) => a.id) || []);
  ...
  const renumberedSubstituents = substituents.map((sub: unknown) => {
    ...
    const newLocant =
      attachedCarbonId !== undefined ? carbonOnlyPositions.get(attachedCarbonId) : subLocant;
