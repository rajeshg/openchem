...
export const P14_3_PRINCIPAL_GROUP_NUMBERING_RULE: IUPACRule = {
  ...
  action: (context: ImmutableNamingContext) => {
    ...
    if (parentStructure.type === "chain" && principalGroups.length === 1) {
      ...
      principalLocants = [
        getPrincipalGroupLocantFromSet(parentStructure, firstPrincipalGroup, optimizedLocants),
      ];
    } else {
      if (process.env.VERBOSE) {
        console.log(
        ...
        console.log(`[P-14.3 MULTI-PRINCIPAL] parentStructure.type=${parentStructure.type}`);
        console.log(
    ...
    const updatedFunctionalGroups = functionalGroups.map((group: FunctionalGroup) => {
      if (group.isPrincipal && principalIdx < principalLocants.length) {
        ...
        if (parentStructure.ringNumberingApplied && group.locants && group.locants.length > 0) {
          if (process.env.VERBOSE) {
            console.log(
      ...
      if (
        ...
        const chainAtomIds = parentStructure.chain.atoms.map((a: Atom) => a.id);
